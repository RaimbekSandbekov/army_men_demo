...
public class movement_by_waypoints1 : MonoBehaviour
{
    ...
    void Start()
    {
        targetWaypoint = waypoints1.points[0];
    }
    ...
    {
        if (wavepointIndex >= waypoints1.points.Length)
        {
            //Destroy(gameObject);
            targetWaypoint = waypoints1.points[0];
            wavepointIndex = 0;
        ...
        {
            wavepointIndex++;
            targetWaypoint = waypoints1.points[wavepointIndex];
        }
    }
}
